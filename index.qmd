...
## 3.3 Model 3: BVAR with Common Stochastic Volatilities

## 3.4 Model 4: BVAR with Common Stochastic Volatilities and MA(1) Gaussian Innovations 
...
# 
# #dim Tx1  variables 
# u_h = array(dim = c(T,1,S))
# h = array(dim = c(T,1,S))
# 
# # dim T by N variables
# u = array(dim = c(N,1,T, S))
# epsilon = array(dim = c(N,1,T, S))
# uvar = array(dim = c(N,N,T,S))
# #dim TxT variables 
# Omega = array(0, dim = c(T,T,S))
# 
# for (s in 1:S){
#   u_h[,,s] = rnorm(n = T, mean = 0, sd = sqrt(sigma_h_squared[s]))
...
#   nu_bar = T + nu_prior
#   #S_bar  = S_prior + t(data)%*%data + t(A_prior)%*%diag(1/diag(V_prior))%*%A_prior - t(A_bar)%*%solve(V_bar)%*%A_bar
#   S_bar = S_prior +  t(data)%*%solve(Omega[,,s])%*%data + t(A_prior)%*%A_prior - t(A_bar)%*%solve(V_bar)%*%A_bar
#   S_bar_inv = solve(S_bar)
#   #Sigma_posterior = rWishart(S, df=nu_bar, Sigma=S_bar_inv)
#   Sigma_posterior = rWishart(S, df=nu_bar, Sigma=S_bar_inv)
#   Sigma_posterior = apply(Sigma_posterior,3,solve)
