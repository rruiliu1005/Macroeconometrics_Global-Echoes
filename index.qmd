...
```{r, echo=FALSE, results='hide', message=FALSE, warning=FALSE, , fig.width=12, fig.height=13, eval = FALSE}
#################################################################################
#################################################################################
#Download Data 
#################################################################################
...
## 3.2 Model 2: Large BVARs model MA(1) Gaussian innovations
...
BVAR_fit <- function(data, p, S, kappa1, kappa2, A_prior, V_prior, S_prior, nu_prior, N, T){
  N = ncol(data)
  T = nrow(data)
  X = matrix(1,T,1)
  for (i in 1:p){
...
posterior_samples = BVAR_fit(data = random_walk_sample, p = p, S = S, kappa1 = kappa1, kappa2 = kappa2, A_prior = A_prior, V_prior = V_prior, S_prior = S_prior, nu_prior = nu_prior)
