...
$$B_H = \frac{p_0(Y)}{p_1(Y)} = \frac{p_1(A_{ij} = 0|Y)}{p_1(A_{ij} = 0)}=\frac{\int p_1(A_{ij=0}|Y, \Sigma)p(\Sigma|Y) d \Sigma}{\int p(A_{ij}=0, \Sigma)d\Sigma}$$
...
- Select the proposal distribution $q(\psi' | \psi^{(t)}) \sim N(\psi^{(t)}, \tau^2)\mathcal{I}(\psi'\in(0,1))$. $\tau^2$ is a tuning parameter that controls the step size.
...
calc_bayes_factor = function(country1, country2, S){
  ...
  row_names = rownames(A_0_mat)
  A_0_mat[row_names %in% country2, country1] = 0
  total_s = dim(posterior_samples_Sigma)[3]
  for (s in 1:S){
    prior_Sigma = MCMCpack::riwish(v = nu_prior, S = S_prior)
    Q_prior = kronecker(prior_Sigma, V_prior)
    # the line below should include the ordinate of a pdf for the restricted parameters only
    prior_prob =  mvtnorm::dmvnorm(x = c(A_0_mat), mean = c(A_prior), sigma = Q_prior, log = TRUE)
    M = c(posterior_samples_A[,,(total_s - s + 1)])
    Q = kronecker(posterior_samples_Sigma[,,(total_s - s + 1)], posterior_V)
    # the line below should include the ordinate of a pdf for the restricted parameters only
    posterior_prob = mvtnorm::dmvnorm(x = c(A_0_mat), mean = M, sigma = Q, log = TRUE)
...
BVAR_MA_fit <- function(data, p, S1, S2, kappa1, kappa2, A_prior, V_prior, S_prior, nu_prior, N){
  ...
  for (s in 1:S){
    ...
    # else{
      ...
      proposal_ratio = RcppTN::rtn(psi, psi_candidate, tau, -1, 1) / RcppTN::rtn(psi_candidate, psi, tau, -1, 1)
