...
```{r}
final_df = inner_join(CPI_data, IR_data, by = 'Time')
#final_df = inner_join(final_df, FDI_data, by = 'Time')
FDI_data_pct <- FDI_data %>%
  mutate(across(c(JP_FDI, CN_FDI, US_FDI, AU_FDI),
                ~ c(NA, 100 * (diff(.) / .[-length(.)])),
                .names = "{.col}"))
FDI_data_pct <- na.omit(FDI_data_pct)

final_df = inner_join(final_df, FDI_data_pct, by = 'Time')

final_df = inner_join(final_df, BOP_data, by = 'Time')
#final_df = inner_join(final_df, adjusted_GDP_data, by = 'Time')
adjusted_GDP_data_pct <- adjusted_GDP_data %>%
  mutate(across(c(JP_GDP, CN_GDP, US_GDP, AU_GDP),
                ~ c(NA, 100 * (diff(.) / .[-length(.)])),
                .names = "{.col}"))

adjusted_GDP_data_pct <- na.omit(adjusted_GDP_data_pct)
final_df = inner_join(final_df, adjusted_GDP_data_pct, by = 'Time')
...
**Original Variables** 
```{r, echo=FALSE, results='hide', message=FALSE, warning=FALSE, , fig.width=12, fig.height=13}
layout <- (IR_plot| CPI_plot) / (BOP_plot | XCH_plot) / (GDP_plot | FDI_plot)
...
results <- sapply(adjusted_GDP_data_pct[2:nrow(adjusted_GDP_data_pct),2:5], perform_adf_test)
results_df <- as.data.frame(t(results), stringsAsFactors = FALSE)
names(results_df) <- c("Dickey-Fuller Statistic", "Lag Order", "P-value")
results_df$Country <- rownames(results_df)
rownames(results_df) <- NULL
...
for (s in 1:S){
  ...
  for (i in 1:h){
    ...
    Y.f               = mvtnorm::rmvnorm(1, mean = x.T%*%A, sigma=Sigma)
    colnames(Y.f) = colnames(x.Ti)
...
JP_CPI_forecast_mean  = apply(BVAR_prediction[,1,],1,mean) 
JP_CPI_forecast_interval = apply(BVAR_prediction[,1,],1,hdi,credMass=0.90)
JP_CPI_forecast_range = range(data[,1],JP_CPI_forecast_interval)
```
...
# plot(1:(length(data[,1])+h),c(data[,1],JP_CPI_forecast_mean), type="l", ylim=JP_CPI_forecast_range, axes=FALSE, xlab="", ylab="", lwd=2)
# specifying x axis
...
#text(x=nrow(data), y=9.65, srt=90, "2023-Q4")
#abline(v=246, col=mcxs1)
#Confidence interval for forecasts
#text(x=nrow(data)+h, y=9.65, srt=90, "2026-12")
...
BVAR_MA_fit <- function(data, p, S1, S2, kappa1, kappa2, A_prior, V_prior, S_prior, nu_prior, N){
  ...
  A_posterior = A_posterior[,,(S1+1):(S1+S2)]
  Sigma_posterior = Sigma_posterior[,,(S1+1):(S1+S2)]
  Omega_posterior = Omega_posterior[,,(S1+2):(S1+S2)]
